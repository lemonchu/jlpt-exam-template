...
构建参数与默认目录见 [README](../README.md#排版与输出)。`--output-dir` 覆盖输出根目录；其中 `N1-<题库>-<题册>.pdf` 为成品，`<题库>-<题册>/` 为工程与报告。`examples/` 不自动覆盖。排版边界见[排版说明](RULE-LAYOUT.md)，维护检查见[架构说明](ARCHITECTURE.md)。
